abstract class ResultBase<O, E>{
  protected readonly __brand_result_base = '__brand_result_base';
  abstract unwrap(): O;
...
type Ok = {
  (): Result<void, never>;
  <O>(o: O): Result<O, never>;
};
type Er = <E>(e: E) => Result<never, E>;

export const ok: Ok = (...arr: unknown[]): any => {
  if (arr.length === 0) return new ResultOk(undefined);
  return new ResultOk(arr[0]);
};
export const er: Er = <E>(e: E): Result<never, E> => new ResultEr(e);
